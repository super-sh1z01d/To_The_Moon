      ...
      {loading ? <p>Загрузка...</p> : (
        <div className="settings">
          ...
          <section>
            ...
            <Field 
              label="🎯 Порог скора для показа в дашборде" 
              type="number" 
              hint="ВИДИМОСТЬ ТОКЕНОВ: Токены с скором ниже этого значения скрываются из основного списка дашборда. ПРИМЕРЫ: 0.2 = показывать много токенов (агрессивный поиск), 0.3 = сбалансированный подход, 0.5 = только лучшие токены (консервативный). ВАЖНО: с жесткой фильтрацией многие токены уже получают 0.0, поэтому можно использовать более низкие пороги." 
              k="min_score" 
              v={vals['min_score']} 
              set={update} 
            />
          ...
          <section>
            ...
            <Field 
              label="🔥 Частота обновления горячих токенов (сек)" 
              type="number" 
              hint="СКОРОСТЬ РЕАКЦИИ НА ЛУЧШИЕ ТОКЕНЫ: Как часто пересчитывать скоры для токенов с высоким потенциалом (статус 'горячие'). ПРИМЕРЫ: 10 сек = быстрая реакция на изменения, 30 сек = экономия ресурсов. ЛОГИКА: горячие токены = высокий скор = нужно отслеживать чаще. С жесткой фильтрацией таких токенов меньше, можно обновлять чаще." 
              k="hot_interval_sec" 
              v={vals['hot_interval_sec']} 
              set={update} 
            />
            <Field 
              label="❄️ Частота обновления холодных токенов (сек)" 
              type="number" 
              hint="ЭКОНОМИЯ РЕСУРСОВ НА СЛАБЫХ ТОКЕНАХ: Как часто пересчитывать скоры для токенов с низким потенциалом (статус 'холодные'). ПРИМЕРЫ: 60 сек = стандарт, 120 сек = больше экономии. ЛОГИКА: холодные токены = низкий скор = можно обновлять реже. С жесткой фильтрацией многие токены получают 0.0, поэтому интервал можно увеличить." 
              k="cold_interval_sec" 
            ...
            <Field 
              ...
              type="number" 
              hint="ОЧИСТКА НЕПЕРСПЕКТИВНЫХ ТОКЕНОВ: Если активный токен показывает низкий скор дольше этого времени, он архивируется (статус 'активный' → 'архив'). ПРИМЕРЫ: 24 часа = быстрая очистка, 48 часов = дать больше шансов. ЦЕЛЬ: освободить ресурсы от токенов, которые потеряли потенциал. С жесткой фильтрацией токены 'умирают' быстрее." 
            ...
            <Field 
              ...
              type="number" 
              hint="ТАЙМАУТ ОЖИДАНИЯ АКТИВАЦИИ: Если токен в статусе 'мониторинг' не активируется (не появляется внешний пул), он архивируется. ПРИМЕРЫ: 48 часов = стандартное ожидание, 72 часа = дать больше времени. ЛОГИКА: 'мониторинг' → ждем внешний пул → 'активный' → начинается скоринг. Если пул не появился = токен неперспективный." 
